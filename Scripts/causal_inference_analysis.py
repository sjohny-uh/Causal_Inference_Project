...
from statsmodels.api import families  # Add this import for GLM families
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('C:/Users/sheri/Downloads/Causal_Inference_Project/Log/causal_inference_analysis.log'),
...
class CausalInferenceAnalyzer:
    """
    Enhanced causal inference analysis for UK stock market data with proper confounder selection
    """
    ...
    def __init__(self, df, target_variable='Target_Direction'):
        ...
        self.confounder_validation_results = {}
    ...
    def test_granger_causality_pair(self, cause_var, effect_var, max_lag=5):
        """
        Test Granger causality between two specific variables

        Parameters:
        ----
        cause_var : str
            Variable that might cause the effect
        effect_var : str
        ...
        Returns:
        ----
        dict : Results with p-value and significance
        """
        try:
            test_data = self.df[[effect_var, cause_var]].dropna()
            ...
            logger.error(f"Error testing Granger causality {cause_var} -> {effect_var}: {str(e)}")
    ...
    def validate_confounder_assumptions(self, confounder, treatment, outcome):
        """
        Validate that a variable satisfies confounder assumptions

        Parameters:
        ----
        confounder : str
            Potential confounder variable
        treatment : str
            Treatment variable
        outcome : str
            Outcome variable
        ...
        """
        # Test 1: Confounder -> Treatment
        conf_to_treat = self.test_granger_causality_pair(confounder, treatment)

        # Test 2: Confounder -> Outcome (check if in causal features)
        conf_to_outcome = confounder in self.causal_features

        # Test 3: Treatment should NOT cause Confounder (avoid post-treatment bias)
        treat_to_conf = self.test_granger_causality_pair(treatment, confounder)

        is_valid = (conf_to_treat['significant'] and 
                   conf_to_outcome and 
                   not treat_to_conf['significant'])

        validation_result = {
            'is_valid': is_valid,
            'conf_to_treatment_p': conf_to_treat['p_value'],
            'conf_to_outcome': conf_to_outcome,
            'treatment_to_conf_p': treat_to_conf['p_value'],
            'conf_to_treatment_sig': conf_to_treat['significant'],
            'treatment_to_conf_sig': treat_to_conf['significant']
        }

        logger.info(f"Confounder validation for {confounder}: Valid={is_valid}, "
                   f"C->T p={conf_to_treat['p_value']:.4f}, "
                   f"T->C p={treat_to_conf['p_value']:.4f}")

        return validation_result

    def correlation_based_confounder_selection(self, treatment_var, outcome_var):
        """
        Select confounders based on correlation with both treatment and outcome
        ...
        Returns:
        ----
        list : Selected confounder variables
        """
        candidates = [f for f in self.causal_features if f != treatment_var]
        confounder_scores = []

        for candidate in candidates:
            if candidate in self.df.columns and treatment_var in self.df.columns:
                try:
                    # Correlation with treatment
                    treat_corr = abs(self.df[candidate].corr(self.df[treatment_var]))

                    # Correlation with outcome  
                    outcome_corr = abs(self.df[candidate].corr(self.df[outcome_var]))

                    # Combined score (both correlations should be significant)
                    combined_score = min(treat_corr, outcome_corr)

                    if treat_corr > 0.1 and outcome_corr > 0.1:  # Minimum thresholds
                        confounder_scores.append({
                            'variable': candidate,
                            'treatment_corr': treat_corr,
        ...
        confounder_scores.sort(key=lambda x: x['combined_score'], reverse=True)
        return [c['variable'] for c in confounder_scores]  # Return all candidates

    def domain_informed_confounder_selection(self, treatment_var, outcome_var):
        """
        Use domain knowledge for confounder selection
        ...
        Returns:
        ----
        list : Selected confounder variables based on domain knowledge
        """
        # Domain knowledge confounder categories
        confounder_categories = {
            'macro_confounders': ['unemployment', 'cpi', 'rate', 'gbp', 'usd'],
            'technical_confounders': ['rsi', 'macd', 'volatility', 'atr', 'bb_'],
            'market_confounders': ['volume', 'return', 'open', 'high', 'low', 'close']
        }

        selected_confounders = []

        # Select all from each category
        for category, keywords in confounder_categories.items():
            category_candidates = [f for f in self.causal_features 
                                 if any(keyword in f.lower() for keyword in keywords) 
                                 and f != treatment_var]

            if category_candidates:
                # Add all candidates from this category
                selected_confounders.extend(category_candidates)

        return selected_confounders

    def select_proper_confounders(self, treatment_var, outcome_var, max_confounders=None):
        """
        Select proper confounders using multiple methods and validation
        ...
        outcome_var : str
            Outcome variable
        max_confounders : int
            Maximum number of confounders to select (None for all)

        Returns:
        ----
        list : Final validated confounders
        """
        print(f"  Selecting proper confounders for {treatment_var} -> {outcome_var}")

        # Method 1: Correlation-based selection
        method1_confounders = self.correlation_based_confounder_selection(treatment_var, outcome_var)

        # Method 2: Domain knowledge selection
        method2_confounders = self.domain_informed_confounder_selection(treatment_var, outcome_var)

        # Combine candidates (remove duplicates, preserve order)
        all_candidates = []
        seen = set()
        for conf in method1_confounders + method2_confounders:
            if conf not in seen:
                all_candidates.append(conf)
                seen.add(conf)

        print(f"    Candidate confounders: {len(all_candidates)}")
        ...
        for candidate in all_candidates:
            validation = self.validate_confounder_assumptions(candidate, treatment_var, outcome_var)
            validation_results[candidate] = validation

            if validation['is_valid']:
                validated_confounders.append(candidate)
                print(f"    {candidate}: Valid confounder")
            else:
                print(f"    {candidate}: Invalid - C->T: {validation['conf_to_treatment_sig']}, "
                     f"C->O: {validation['conf_to_outcome']}, T->C: {validation['treatment_to_conf_sig']}")

        # Store validation results for reporting
        self.confounder_validation_results[treatment_var] = validation_results

        # Select final confounders
        if max_confounders is None:
            final_confounders = validated_confounders
        else:
            final_confounders = validated_confounders[:max_confounders]

        # If we don't have enough validated confounders and max_confounders is set, add best candidates
        if max_confounders is not None and len(final_confounders) < max_confounders:
            remaining_candidates = [c for c in all_candidates if c not in final_confounders]
            additional_needed = max_confounders - len(final_confounders)
            final_confounders.extend(remaining_candidates[:additional_needed])

        print(f"    Selected {len(final_confounders)} confounders: {final_confounders}")
    ...
    def dowhy_causal_analysis(self):
        """
        Enhanced DoWhy framework for causal effect estimation with proper confounder selection
        """
        print(f"\n{'='*60}")
        print("ENHANCED DOWHY CAUSAL ANALYSIS")
        print(f"{'='*60}")
        logger.info("Starting enhanced DoWhy causal analysis with proper confounder selection")

        # Use all significant features for detailed DoWhy analysis
        ...
        for treatment_var in top_features:
            try:
                ...
                # ENHANCED: Select proper confounders using validation (no limit)
                confounders = self.select_proper_confounders(treatment_var, self.target, max_confounders=None)

                if not confounders:
                    print(f"  No valid confounders found for {treatment_var}")
                    logger.warning(f"No valid confounders found for {treatment_var}")
                    continue
                ...
                # Estimate causal effect using multiple methods with proper GLM family
                methods = [
                    ("backdoor.linear_regression", {}),
                    ("backdoor.generalized_linear_model", {"glm_family": families.Binomial()})
                ]
                ...
                for method_name, method_params in methods:
                    try:
                        ...
                        print(f"  {method_name}: {estimate.value:.4f}")
                        logger.info(f"DoWhy estimate for {treatment_var} using {method_name}: {estimate.value:.4f}")
                    except Exception as e:
                        print(f"  {method_name}: Failed ({str(e)})")
                        logger.error(f"DoWhy estimation failed for {treatment_var} using {method_name}: {str(e)}")
                ...
                # 2. Random common cause test - FIXED VERSION
                try:
                    # Get the first successful estimate for robustness testing
                    first_estimate = None
                    for method_name, estimate_data in estimates.items():
                        if 'value' in estimate_data:
                            # Create a mock estimate object with the required attributes
                            class MockEstimate:
                                def __init__(self, value):
                                    self.value = value
                            first_estimate = MockEstimate(estimate_data['value'])
                            break
                    ...
                    if first_estimate is not None:
                        ...
                        # Handle both new_effect attribute and value attribute
                        refutation_value = getattr(refutation, 'new_effect', getattr(refutation, 'value', None))
                        if refutation_value is not None:
                            robustness_results['random_common_cause'] = refutation_value
                            print(f"  Random common cause test: {refutation_value:.4f}")
                            logger.info(f"Random common cause test for {treatment_var}: {refutation_value:.4f}")
                        else:
                            print(f"  Random common cause test: Could not extract refutation value")
                            logger.warning(f"Random common cause test for {treatment_var}: Could not extract refutation value")
                    else:
                        print(f"  Random common cause test: No valid estimate available")
                        logger.warning(f"Random common cause test for {treatment_var}: No valid estimate available")
                ...
                dowhy_results[treatment_var] = {
                    ...
                    'confounder_validation': self.confounder_validation_results.get(treatment_var, {})
                }
        ...
        logger.info(f"Enhanced DoWhy causal analysis completed for {len(dowhy_results)} variables")
    ...
    def sensitivity_analysis(self):
        ...
        for period_name, period_mask in periods:
            ...
            # Test all causal relationships in this period
            period_results = []
    ...
    def prepare_modeling_dataset(self):
        ...
        Keep lag and diff feature names for realistic prediction
        """
        print(f"\n{'='*60}")
        ...
        # Keep causal features with their original lag and diff names
        if hasattr(self, 'causal_features') and self.causal_features:
            print(f"Found {len(self.causal_features)} causal features from analysis")
            logger.info(f"Found {len(self.causal_features)} causal features")

            # Keep original causal feature names (including lag and diff)
            original_causal_features = []
            for causal_feature in self.causal_features:
                if causal_feature in self.modeling_df.columns:
                    original_causal_features.append(causal_feature)

            print(f"Keeping {len(original_causal_features)} causal features with their original names:")
            for orig_feature in original_causal_features[:10]:  # Show first 10
                print(f"   - {orig_feature}")
            if len(original_causal_features) > 10:
                print(f"   ... and {len(original_causal_features) - 10} more")

            logger.info(f"Keeping {len(original_causal_features)} causal features with original names")
            ...
            if hasattr(self, 'granger_results') and not self.granger_results.empty:
                # Map importance scores to causal features
                causal_scores = {}
                for _, row in self.granger_results.iterrows():
                    causal_feat = row['feature']
                    if causal_feat in original_causal_features:
                        # Convert p-value to importance score (lower p-value = higher importance)
                        importance = -np.log10(max(row['min_p_value'], 1e-10))
                        causal_scores[causal_feat] = importance

                self.modeling_df.attrs['causal_importance_scores'] = causal_scores

            # Get all available features (excluding targets and metadata)
            exclude_cols = ['Date', 'Ticker', 'Target_Direction', 'Target_5D_Return',
                           'Target_10D_Return', 'Target_Excess_Return']

            # Keep all features including lag and diff features
            all_features = []
            for col in self.modeling_df.columns:
                if col not in exclude_cols:
                    all_features.append(col)

            # Separate causal and non-causal features
            non_causal_features = [f for f in all_features if f not in original_causal_features]

            # Categorize causal features by type
            macro_causal = [f for f in original_causal_features if any(x in f.lower() for x in
                           ['unemployment', 'cpi', 'rate', 'gbp', 'usd'])]
            technical_causal = [f for f in original_causal_features if any(x in f.lower() for x in
                               ['rsi', 'macd', 'bb_', 'atr', 'sma', 'ema', 'stoch'])]
            price_causal = [f for f in original_causal_features if any(x in f.lower() for x in
                           ['return', 'volatility', 'volume', 'open', 'high', 'low', 'close'])]
            other_causal = [f for f in original_causal_features 
                           if f not in macro_causal + technical_causal + price_causal]

            print(f"\nFeature breakdown for modeling:")
            print(f"   Total features available: {len(all_features)}")
            print(f"   Causal features (with lag/diff names): {len(original_causal_features)}")
            print(f"    - Macroeconomic: {len(macro_causal)}")
            print(f"    - Technical: {len(technical_causal)}")
            print(f"    - Price-based: {len(price_causal)}")
            print(f"    - Other: {len(other_causal)}")
            print(f"   Non-causal features: {len(non_causal_features)}")
            print(f"   Keeping lag/diff feature names for modeling")

            logger.info(f"Feature breakdown - Total: {len(all_features)}, Causal: {len(original_causal_features)}, Non-causal: {len(non_causal_features)}")

            # Store feature lists as metadata
            self.modeling_df.attrs['causal_features'] = original_causal_features
            ...
            self.modeling_df.attrs['other_causal_features'] = other_causal

            # Create priority rankings based on importance scores
            if 'causal_importance_scores' in self.modeling_df.attrs:
                causal_scores = self.modeling_df.attrs['causal_importance_scores']
                priority_features = sorted(causal_scores.keys(), 
                                         key=lambda x: causal_scores[x], reverse=True)
                self.modeling_df.attrs['feature_priority_ranking'] = priority_features

                print(f"\nTop 10 most important causal features for modeling:")
                for i, feat in enumerate(priority_features[:10], 1):
                    score = causal_scores[feat]
                    print(f"   {i:2d}. {feat:<35} (importance: {score:.2f})")
            ...
            exclude_cols = ['Date', 'Ticker', 'Target_Direction', 'Target_5D_Return',
                           'Target_10D_Return', 'Target_Excess_Return']
            all_features = [col for col in self.modeling_df.columns if col not in exclude_cols]

            self.modeling_df.attrs['causal_features'] = []
            self.modeling_df.attrs['non_causal_features'] = all_features
        ...
        self.modeling_df.attrs['modeling_approach'] = 'keep_lag_diff_names'
        self.modeling_df.attrs['confounder_selection_method'] = 'enhanced_validation'
        ...
        print(f"\nEnhanced dataset saved: {output_filename}")
        ...
        metadata = {
            ...
            'other_causal_features': self.modeling_df.attrs.get('other_causal_features', []),
            ...
            'confounder_validation_results': self.confounder_validation_results,
            'target_variable': self.target,
            'modeling_approach': 'keep_lag_diff_names',
            'confounder_selection_method': 'enhanced_validation',
            'analysis_summary': {
    ...
    def generate_causal_report(self):
        """
        Generate comprehensive causal inference report with confounder validation details
        """
        print(f"\n{'='*80}")
        print("ENHANCED CAUSAL INFERENCE ANALYSIS - COMPREHENSIVE REPORT")
        print(f"{'='*80}")
        ...
        print(f"   Significant causal relationships: {len(self.causal_features)}")
        print(f"   Confounder selection method: Enhanced validation")
        ...
        # Enhanced DoWhy results summary with confounder details
        if hasattr(self, 'dowhy_results') and self.dowhy_results:
            print(f"\n ENHANCED CAUSAL EFFECT ESTIMATES (DoWhy with Validated Confounders):")
            for treatment, results in self.dowhy_results.items():
                print(f"\n   Treatment: {treatment}")
                print(f"   Confounders used: {results.get('confounders_used', [])}")

                # Show confounder validation details
                if 'confounder_validation' in results:
                    print(f"   Confounder validation:")
                    for conf, validation in results['confounder_validation'].items():
        ...
        print(f"   - No limit on significant features for DoWhy analysis")
        print(f"   - All significant features used in sensitivity analysis")
        print(f"   - All confounders selected (no limit)")
        print(f"   - Lag and diff feature names preserved in modeling dataset")

        logger.info("Enhanced causal inference report generation completed")
...
# Usage example functions (unchanged)
def run_causal_analysis(df, target='Target_Direction'):
    ...
    # Step 3: Enhanced DoWhy Analysis
    ...
    print(f"\n ENHANCED CAUSAL INFERENCE ANALYSIS COMPLETED!")
    ...
    print(f"   Enhanced with proper confounder selection and validation")
    logger.info(f"Enhanced causal inference analysis completed - {len(analyzer.causal_features)} significant features identified")

    return analyzer, modeling_df

def run_causal_analysis_with_modeling_output(df, target='Target_Direction'):
    """
    Convenience function that returns both analyzer and modeling-ready dataframe
    ...
    """
    logger.info("Running enhanced causal analysis with modeling output")
