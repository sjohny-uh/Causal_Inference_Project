...
from data_preproc_feature_sel import UKStockAnalyzer
from advanced_analytics import AdvancedAnalytics
from causal_inference_analysis import CausalInferenceAnalyzer

# Import prediction module
from stock_market_prediction import run_causal_modeling_analysis
...
def main():
    ...
    try:
        ...
        #pipeline, results = run_real_data_modeling_analysis(df=modeling_df)
        pipeline, results = run_causal_modeling_analysis(modelling_df=modeling_df, causal_metadata_path='causal_analysis_metadata.json')

        print(f"\n PREDICTION ANALYSIS COMPLETED!")
        if results is not None:
            print(f"   Trained {len(results)} model configurations")
            print(f"   Best F1 Score: {results['F1_Score'].max():.4f}")
            print(f"   Best Sharpe Ratio: {results['Sharpe_Ratio'].max():.4f}")

        return analyzer, causal_inf, pipeline, results
...
if __name__ == "__main__":
    analyzer, causal_inf, pipeline, results = main()