    ...
    def calculate_technical_indicators(self):
        ...
        def apply_technical_analysis(group):
            ...
            try:
                ...
                group['Drawdown'] = (cumulative_returns - running_max) / running_max
                group['Max_Drawdown'] = group['Drawdown'].rolling(window=252).min()
                excess_returns = group['Daily_Return'] - (group['Rate'] / 100 / 252)
                group['Sharpe_Ratio'] = excess_returns.rolling(window=252).mean() / group['Daily_Return'].rolling(window=252).std() * np.sqrt(252)
